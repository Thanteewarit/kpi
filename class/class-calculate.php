...
function asc($v1,$v2,$v3,$v4,$v5,$isVlue)
{
    ...
    if(CheckValue($v2))
    {
        if(floatval(SplitValue($v2)[0])>floatval(SplitValue($v2)[1]))
        {
            if($isVlue<=$v1)
            {
                return 1;
            }
            elseif($isVlue<=floatval(SplitValue($v2)[0]) && $isVlue>=floatval(SplitValue($v2)[1]))
            {
                return 2;
            }
            elseif($isVlue<=floatval(SplitValue($v3)[0]) && $isVlue>=floatval(SplitValue($v3)[1]))
            {
                return 3;
            }
            elseif($isVlue<=floatval(SplitValue($v4)[0]) && $isVlue>=floatval(SplitValue($v4)[1]))
            {
                return 4;
            }
            elseif($isVlue>=$v5)
...
function desc($v1,$v2,$v3,$v4,$v5,$isVlue)
{
    ...
    if(CheckValue($v2))
    {
        ...
        if(floatval(SplitValue($v2)[0])>floatval(SplitValue($v2)[1]))
        {
            ...
            else if($isVlue<=floatval(SplitValue($v2)[0]) && $isVlue>=floatval(SplitValue($v2)[1]))
            {
                echo "2";
                return 2;
            }
            else if($isVlue<=floatval(SplitValue($v3)[0]) && $isVlue>=floatval(SplitValue($v3)[1]))
            {
                return 3;
            }
            else if($isVlue<=floatval(SplitValue($v4)[0]) && $isVlue>=floatval(SplitValue($v4)[1]))  
            {
                return 4;
            }
            else if($isVlue<=$v5)
...
else
{
    ...
    {
        $table="tb_evaluation";
        $valuei="evaluation_value='".$_POST['id']."', evaluation_total_score = '".number_format($score*$_POST['Weight']/100, 2)."', evaluation_value_score = '$score' 
        where kpi_master_id='".$_POST['kpi_master_id']."'
...
if($_POST['Mode']=="In_scoreDepartment")
{
    ...
    $valuei="SELECT * FROM tb_evaluation_all WHERE evaluation_code = '".$_POST['code']."'";
    $idHotspital = c_scelectOne($valuei);

    $valuei="
    SELECT  staff_code   FROM tb_staff 
    WHERE   staff_hospital_id               =   '".$idHotspital['evaluation_all_hospital_id']."'
    AND     staff_director_assistant_id     =   '".$idHotspital['evaluation_all_director_assistant_id']."'
    AND     staff_division_director_id      =   '".$idHotspital['evaluation_all_division_director_id']."'
    AND     staff_division_manager_head_id  =   '".$idHotspital['evaluation_all_division_manager_head_id']."'
    AND     staff_division_manager_sub_id   =   '".$idHotspital['evaluation_all_division_manager_sub_id']."'
    AND     staff_depatment_head_id         =   '".$idHotspital['evaluation_all_depatment_head_id']."'
    ";
